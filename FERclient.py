...
parser.add_argument("-e", "--environment", help="which my- env been used", type=str, default="CartPoleSwingUp-v0")
...
class CartPoleSwingUpClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        ...
        # print([key for key, value in self.model.policy.state_dict().items() if "features_extractor" in key])
        # sys.exit()
        policy_state = [value.cpu().numpy() for key, value in self.model.policy.state_dict().items()]
        # policy_state = [value.cpu().numpy() for key, value in self.model.policy.state_dict().items() if "features_extractor" in key]
        return policy_state

    def set_parameters(self, parameters):
        # parameters = [th.tensor(v) for v in parameters]
        # features_extractor_keys = [key for key in self.model.policy.state_dict().keys() if "features_extractor" in key]
